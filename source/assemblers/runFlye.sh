...
################################################################################
# Use: Use Flye with plasmid and metagenomics settings to build an assembly from 
#      input reads
# input:
...
#    -o: string with the output direcotry to save the output from flye to
#            - includes intermediate files made by flye
#        Default: prefix--readDepthx-flye
...
# output:
#     Dir:
#         Name: User provided (-o) [prefix--readDepthx-flye]
#         Location: Current working directory (unless path provided in name)
#         Contents: Files produced by flye
#     File: 
#         Name: prefix--readDepthx-flye.fasta 
#         Location: output directory (-o) [prefix--readDepthx-flye]
#         Contents: The un-polished flye assembly
#     File:
#         Name: prefix--readDepthx-flye--log.txt
#         Location: output directory (-o) [prefix--readDepthx-flye]
#         Contents:
...
prefixStr="prefix";
assemblerStr="flye"; # used for file names (here so I can easly change later)
...
while getopts ':d:h:o:p:r:t:x' option; do
...
# put the flye command I will run in the log file
printf "Assembler command:\n\t%s --plasmids --meta" "$assemblerStr" >> \
	"$logStr";
printf " --threads %i --nano-raw %s --out-dir %s\n\n" \
	$threadsInt "$readsStr" "$outDirStr" >> "$logStr";
    # broke into two commands to reduce to 80 char

# run flye (Not using eval for this since user input is used)
/usr/bin/time -o "$timeLogStr" -a \
	...
	"$(command -v "$assemblerStr")" --plasmids --meta --threads $threadsInt \
		--nano-raw "$readsStr" --out-dir "$outDirStr";
...
# re-name the assembly
mv "$outDirStr/assembly.fasta" "$outFileStr.fasta";
