...
################################################################################
# Use: Use Canu with plasmid and metagenomics settings to build an assembly from 
#      input reads
# input:
...
#    -o: string with the output direcotry to save the output from canu to
#            - includes intermediate files made by canu
#        Default: prefix--readDepthx-canu
...
# output:
#     Dir:
#         Name: User provided (-o) [prefix--readDepthx-canu]
#         Location: Current working directory (unless path provided in name)
#         Contents: Files produced by canu
#     File: 
#         Name: prefix--readDepthx-canu.fasta 
#         Location: output directory (-o) [prefix--readDepthx-canu]
#         Contents: The un-polished canu assembly
#     File:
#         Name: prefix--readDepthx-canu--log.txt
#         Location: output directory (-o) [prefix--readDepthx-canu]
#         Contents:
...
prefixStr="prefix";
assemblerStr="canu"; # used for file names (here so I can easly change later)
...
versionStr="$($assemblerStr --version | sed 's/.* //)"; # assembler version 

# help message
helpStr="$(basename "$0") [-h] -r <fastq> -g <genomeSize>
    Use: Build an assembly using input reads and Canu
    Input:
...
printf "\t-g %s\n\t-r %s\n\t-d %s\n\t-o %s\n\t-p %s\n\t-t %i\n\n" \
	"$genSizeStr" "$readsStr" "$depthStr" "$outDirStr" "$prefixStr" \
	$threadsInt >> "$logStr";
...
# put the canu command I will run in the log file
printf "Assembler command:\n\t%s -p canu stopOnLowCoverage=0" "$assemblerStr" \
	>> "$logStr";
printf " genomesize=%s --nanopore-raw %s -d %s\n\n" \
	"$genSizeStr" "$readsStr" "$outDirStr" >> "$logStr";
    # broke into two commands to reduce to 80 char

# run canu (Not using eval for this since user input is used)
/usr/bin/time -o "$timeLogStr" -a \
	...
	"$(command -v "$assemblerStr")" -p "canu" stopOnLowCoverage=0 \
		genomesize="$genSizeStr" --nanopore-raw "$readsStr" -d "$outDirStr";
...
# if no errors append the name of the output directory & assembly to the log
if [[ -f "$outFileStr.fasta" ]]; then # if made the assembly
    sed -i'' "s/\(Assembly:.*\)/\1\t$nameStr.fasta/" "$logStr";
        # -i allows me to have sed output to the orginal file
else
