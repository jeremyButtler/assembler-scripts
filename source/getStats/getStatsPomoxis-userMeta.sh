...
#         Default: 16
#     -d: integer with read depth
#     -a: String with the assembler
#     -w: String with the polisher
#     -s: String with the suffix (id)
#     
...
depthStr="NA"; # get from assmebly file name
asmbStr="NA"; # get from assembly file name
polishStr="NA"; # get from assembly file name
idStr="NA"; # suffix from the file name (My runAsmbbler script uses numbers)
...
while getopts ':i:h:m:p:r:t:d:a:w:s:x' option; do
...
nameStr="$prefixStr--""$depthStr""x-$asmbStr-$polishStr--$idStr-pomoxis";
outStr="$nameStr";
...
# if need to make a spreed sheet with the Q scores and coverage
    ...
    awk -f "../$scriptDirStr/pomoxisIsoletQ.awk" -v depthStr="$depthStr" \
		...
		-v verStr="$versionStr" -v idStr=$idStr \
		< "$nameStr-summ.txt" > "$nameStr-Q.csv";
